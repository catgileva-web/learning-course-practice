...
For line chart, you can increase the interaction area by using slots. Detailed explanations are available in a dedicated line interaction demo.
...
Having a controlled highlight lets you control it in multiple charts at the same time. You need to ensure that the series has the same ids and the data is in the same order.
